...
const titleBase = 'BloomBridge';
const defaultTitle = `${titleBase}`;
const defaultDescription = `BloomBridge is a web application that allows you to transfer wrapped tokens between
ShimmerEVM and several other blockchain networks by interacting with the decentralized smart contracts of LayerZero 
deployed on each of those networks. BloomBridge supports connections from ShimmerEVM to Ethereum, Avalanche C-Chain, 
...
const keywords = `BloomBridge, BloomBridge, BloomBridge, Shimmer, shimmer, layerzero, blockchain, crypto, message, transaction, omnichain, bridge, ${generateKeywords(defaultDescription).join(", ")}`;
...
export const CustomHtmlHead = (props: CustomHtmlHeadProps) => {
    ...
    return (
        <Head>
            ...
            <meta name='author' content={'BloomBridge'}/>
            <meta name='designer' content={'BloomBridge'}/>
            <meta name='publisher' content={'BloomBridge'}/>
            ...
            {/*<meta name='twitter:site' content='@BloomBridge'/>*/}
            ...
            <meta name="application-name" content="BloomBridge"/>
        </Head>
    );
};
